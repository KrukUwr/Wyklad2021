...
svm7v <- svm(IfSigned ~ D_ContractDateToImportDate + NoPay + NoCalls + 
    PrcVisitsCl + IfMan + IfReach + TOA, 
  data=casesTrn, kernel="radial", cost=10, scale=TRUE)
...
#    "IfMan", "IfReach", "PrcVisitsCl", "D_ContractDateToImportDate")], 
...
svm7vTuned <- svm(IfSigned ~ D_ContractDateToImportDate + NoPay + NoCalls + 
    PrcVisitsCl + IfMan + IfReach + TOA, 
  data=casesTrn, kernel="radial", cost=50, gamma=0.1, scale=TRUE)
...
frmCl <- as.formula("IfSigned + IfNotSigned ~ D_ContractDateToImportDate + 
NoPay + NoCalls + PrcVisitsCl + IfMan + IfReach + TOA")
...
nnetFitClPred <- compute(nnetFitCl, casesTstStd[, .SD, .SDcols=c(
  "D_ContractDateToImportDate", "NoPay", "NoCalls", "PrcVisitsCl", "IfMan", 
  "IfReach", "TOA")])
...
frm <- as.formula("IfSigned ~ D_ContractDateToImportDate + NoPay + NoCalls + 
    PrcVisitsCl + IfMan + IfReach + TOA")
